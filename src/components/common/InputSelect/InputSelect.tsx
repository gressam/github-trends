...
export const InputSelect = (props: InputSelectType): JSX.Element => {
  ...
  const getClearItem = useCallback(
    () => ({ value: props.clearText || '', label: props.clearText || '' }),
    [props.clearText]
  );
  ...
  useEffect(() => {
    if (searchValue) {
      ...
      setItems(
        selectedItem && props.addClearLabel ? [getClearItem(), ...props.items] : props.items
      );
    }
  ...
  const handleItemSelect = (item: ISelectItem) => {
    ...
    setItems((prevItems: ISelectItem[]): ISelectItem[] => {
      if (
        prevItems.findIndex((item: ISelectItem): boolean => item.value === props.clearText) >= 0 ||
        !props.addClearLabel
      ) {
        return prevItems;
      } else {
  ...
  return (
    <div ref={ref} className="input-select">
      <div className="input-select-title" onClick={() => setIsOpen(!isOpen)}>
        {props.title} <span>{selectedItem?.label || 'Any'}</span>
      </div>
      {isOpen && (
