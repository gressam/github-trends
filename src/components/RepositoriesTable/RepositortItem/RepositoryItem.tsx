...
export const RepositoryItem = (props: RepositoryItemProps): JSX.Element => {
  ...
  const getIconByDeveloper = (user: IShortDeveloperInfo) => {
    return <img key={user.username} alt="developer" src={user.avatar} />;
  };

  const getStarIcon = () => {
    return (
      <svg height="16" width="16" className="ico">
        <path
          fillRule="evenodd"
  ...
  return (
    <div className="repository-item">
      <div className="repository-item-header">
        ...
        <Button title="Star" icon={getStarIcon()} />
      </div>
      <div className="repository-item-description">{item.description}</div>
      <div className="repository-item-info">
        <div className="short-info">
          <div className="language">{item.language}</div>
          <div className="stars">
            {getStarIcon()} {item.totalStars}
          </div>
          <div className="forks">
        ...
        <div className="time-stars">
          {getStarIcon()} {item.starsSince} stars this {item.since}
        </div>
      </div>
    </div>
  );
